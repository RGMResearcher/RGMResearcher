...
graph_type preferred_attachment(unsigned, unsigned);
...
private:
	...
	void dfs(unsigned);
public:
	const std::vector<unsigned>& sort(const graph_type& grph)
	{
		gr = &grph;
		used.clear();
		ans.clear();
...
struct result_graph_generator
{
	...
	graph_type gr; // сам граф
	unsigned loop_count;

	result_graph_generator(): loop_count(0){}
};

std::map<unsigned, double> page_rank(const graph_type&, const double& c = 0.85,
...
//
double cl_coef(const graph_type&);
graph_type cl_to_gr(const cluster&, const graph_type&);

void print(const graph_type&, std::ostream&);
...
// Ассортативность
double assort(const graph_type&);
...
std::vector<unsigned> diameter(const graph_type&, unsigned ret_size = 0);
